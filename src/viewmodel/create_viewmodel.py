...
class CreateViewModel(BaseViewModel):
    ...
    def validate_input(self, bare_repo_path, work_repo_path, repo_name):
        if not bare_repo_path:
            return False, vk.LocalizedText("bare_repo_path_empty")
        if not work_repo_path:
            return False, vk.LocalizedText("work_repo_path_empty")
        if not repo_name:
            return False, vk.LocalizedText("repo_name_empty")
        return True, vk.LocalizedText("input_validation_success")
    
    def create_repo(self, bare_repo_path, work_repo_path, repo_name, gitignore_template, enable_lfs, gitattributes_template):
        try:
            valid, message = self.validate_input(bare_repo_path, work_repo_path, repo_name)
            if not valid:
                self.on_create_completed.broadcast(False, message)
                return
            ...
            if not valid:
                self.on_create_completed.broadcast(False, self._to_localized_text(message))
                return
            
            compatible, version_message = self.git_manager.is_version_compatible()
            if not compatible:
                self.on_create_completed.broadcast(False, self._to_localized_text(version_message))
                return
            
            if not os.path.isdir(bare_repo_path):
                self.on_create_completed.broadcast(False, self._to_localized_text(f"bare_repo_path_invalid: {bare_repo_path}"))
                return
            
            if not os.path.isdir(work_repo_path):
                self.on_create_completed.broadcast(False, self._to_localized_text(f"work_repo_path_invalid: {work_repo_path}"))
                return
            
            bare_repo_full_path = os.path.join(bare_repo_path, f"{repo_name}.git")
            if os.path.exists(bare_repo_full_path):
                self.on_create_completed.broadcast(False, self._to_localized_text(f"bare_directory_exists: {bare_repo_full_path}"))
                return
            
            work_repo_full_path = os.path.join(work_repo_path, repo_name)
            if os.path.exists(work_repo_full_path):
                self.on_create_completed.broadcast(False, self._to_localized_text(f"work_directory_exists: {work_repo_full_path}"))
                return
            
            code, stdout, stderr = self.git_manager.create_bare_repo(bare_repo_full_path)
            if code != 0:
                self.on_create_completed.broadcast(False, self._to_localized_text(f"create_bare_repo_failed: {stderr}"))
                return
            
            code, stdout, stderr = self.git_manager.clone_repo(bare_repo_full_path, work_repo_full_path)
            if code != 0:
                self.on_create_completed.broadcast(False, self._to_localized_text(f"clone_repo_failed: {stderr}"))
                return
            ...
            if enable_lfs:
                ...
                if code != 0:
                    self.on_create_completed.broadcast(False, self._to_localized_text(f"enable_lfs_failed: {stderr}"))
                    return
            ...
            self.on_create_completed.broadcast(True, vk.LocalizedText("repo_creation_success"))
        except Exception as e:
            self.on_create_completed.broadcast(False, self._to_localized_text(f"repo_creation_error: {str(e)}"))
    ...
    def save_result(self, text, color_type):
        self.local_data_manager.set_create_result(text, color_type)
